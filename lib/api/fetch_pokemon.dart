...
IOEither<String, int> _validateUserPokemonId(String pokemonId) =>
    _parseStringToInt(
      ...
      (intPokemnoId) => IOEither.fromPredicate(
        intPokemnoId,
        (id) =>
...
TaskEither<String, Pokemon> fetchPokemonFromUserInput(String pokemonId) =>
    TaskEither.Do((_) async {
      final validatePokemonId = await _(_validateUserPokemonId(
        pokemonId,
      ).toTaskEither());
      return _(fetchPokemon(validatePokemonId));
    });

TaskEither<String, Pokemon> fetchRandomPokemon = TaskEither.Do((_) async {
  final pokemonId = await _(
      randomInt(Constants.minimumPokemonId, Constants.maximumPokemonId + 1)
          .toTaskEither());
